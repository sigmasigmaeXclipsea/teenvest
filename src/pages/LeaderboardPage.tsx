...
const LeaderboardPage = () => {
  ...
  const top3 = enrichedLeaderboard.filter((entry: any) => entry.rank <= 3);
  ...
  const allRankedEntries = enrichedLeaderboard.filter((entry: any) => entry.rank <= 100);
  ...
  // Render the appropriate entry based on mode
  const renderEntry = (entry: any, showSeparator = false) => {
    if (mode === 'portfolio') {
      return renderPortfolioEntry(entry as LeaderboardEntry, showSeparator);
    }
    return renderRankEntry(entry as RankLeaderboardEntry, showSeparator);
  };
